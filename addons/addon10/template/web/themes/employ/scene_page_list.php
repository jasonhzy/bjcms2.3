		...
		<?php  if(is_array($scene_page_list)) { foreach($scene_page_list as $item) { ?>
				<tr>
					...
					<td class="text-center">
									...
									<?php if($row['m_type']==1) {?>场景1(双图片)<?php }elseif($row['m_type']==2) {?>
								场景2(双图片)<?php }elseif($row['m_type']==3) {?>
									场景3(双图片)<?php }elseif($row['m_type']==4) {?>
									场景4(上下图)<?php }elseif($row['m_type']==5) {?>
									场景4(优酷视频)<?php }elseif($row['m_type']==8) {?>
									多图多文字<?php }else{ ?>纯图片式
								<?php }?>
			</td>
